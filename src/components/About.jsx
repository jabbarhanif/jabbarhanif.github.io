...
function CanvasSkeleton() {
    return (
        <div className="relative rounded-2xl overflow-hidden ring-1 ring-white/10 bg-gradient-to-br from-slate-800/60 to-slate-900/60">
            <div className="aspect-[4/3] sm:aspect-[3/2] lg:aspect-square w-full" />
...
export default function About() {
    ...
    return (
        <div className="py-16 sm:py-20">
            <div className="grid lg:grid-cols-2 gap-y-12 gap-x-10 items-center">
                {/* teks dulu di mobile agar konten tidak “kosong” di atas */}
                <div>
                    ...
                    <ul className="mt-6 flex flex-wrap gap-2">
                        {["React", "Three.js", "WebGL", "TypeScript", "Tailwind CSS", "Realtime", "Shaders", "UX Motion"].map((tag) => (
                            <li key={tag} className="text-xs sm:text-sm rounded-full px-3 py-1 ring-1 ring-white/15 bg-white/5 text-white/80">
                                {tag}
                            </li>
                        ))}
                    </ul>
                    ...
                    <div className="mt-8 flex flex-wrap gap-3">
                        ...
                        <a href="#contact" className="inline-flex items-center rounded-xl px-4 py-2 text-sm font-semibold bg-white/5 hover:bg-white/10 ring-1 ring-white/15 text-white/90">
                            Hubungi Saya
                        </a>
                    </div>
                </div>
                    ...
                    <div className="relative rounded-2xl overflow-hidden ring-1 ring-white/10 bg-gradient-to-br from-slate-800/60 to-slate-900/60">
                        <div className="aspect-[4/3] sm:aspect-[3/2] lg:aspect-square w-full">
                            <Suspense fallback={<CanvasSkeleton />}>
                                <Scene3D active={inView} dpr={dpr} className="h-full w-full select-none" />
                            </Suspense>
                        </div>

                        <div className="absolute bottom-3 left-3 text-xs sm:text-sm text-white/70">Realtime 3D Preview</div>
                        {/* kurangi area glow agar tidak overflow ke kanan */}
